...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="page-header mb-0"><i class="bi bi-pencil me-2"></i>Edit Trip</h1>
...
<div class="glass-card p-4" style="max-width: 600px;">
    <form action="{{ route('admin.trips.update', $trip) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="bus_id" class="glass-select form-control @error('bus_id') is-invalid @enderror" required>
                ...
                @foreach($buses as $bus)
                    <option value="{{ $bus->id }}" {{ old('bus_id', $trip->bus_id) == $bus->id ? 'selected' : '' }}>{{ $bus->plate_number }} ({{ $bus->capacity }} seats)</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="route_id" class="glass-select form-control @error('route_id') is-invalid @enderror" required>
                ...
                @foreach($routes as $route)
                    <option value="{{ $route->id }}" {{ old('route_id', $trip->route_id) == $route->id ? 'selected' : '' }}>{{ $route->origin }} → {{ $route->destination }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="date" name="date" class="glass-input form-control @error('date') is-invalid @enderror"
                   value="{{ old('date', $trip->date->format('Y-m-d')) }}" required>
        ...
        <button type="submit" class="btn btn-primary-glass"><i class="bi bi-save me-1"></i>Update Trip</button>
    </form>
</div>
@endsection
